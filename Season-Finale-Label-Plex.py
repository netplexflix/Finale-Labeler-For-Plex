...
SONARR_API_KEY = 'xxxxxxxxxx'  						# Replace with your Sonarr API Key found under settings => General

# --------------------------#
# Configuration (Plex)      #
# --------------------------#
PLEX_URL = "http://localhost:32400"     # Edit if needed
PLEX_TOKEN = "xxxxxxxxxx"     			# Replace with your Plex Token (see https://support.plex.tv/articles/204059436-finding-an-authentication-token-x-plex-token/)
...
GENRES_TO_SKIP = ["Talkshow","Talk Show","News","Stand-Up","Awards Show"]
...
def build_plex_id_map(plex_shows):
    ...
    for show_obj in plex_shows:
        try:
            ...
            print(f"ERROR: Failed to reload show '{show_obj.title}': {e}")
        ...
        # Debugging: Print genres for each show being added to the map
        genres = [genre.tag for genre in show_obj.genres]
    return id_map
